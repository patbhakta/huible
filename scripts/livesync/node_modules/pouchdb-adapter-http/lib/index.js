...
function hasUrlPrefix(opts) {
  ...
  const protocol = pouchdbUtils.parseUri(opts.prefix).protocol;
...
function getHost(name, opts) {
  ...
  const uri = pouchdbUtils.parseUri(name);
...
function HttpPouch(opts, callback) {
  ...
  const ourFetch = async function (url, options) {
    ...
    if (opts.auth || host.auth) {
      ...
      const token = pouchdbBinaryUtils.btoa(unescape(encodeURIComponent(str)));
    ...
    const fetchFun = opts.fetch || pouchdbFetch.fetch;
    return await fetchFun(url, options);
  };

  function adapterFun(name, fun) {
    return pouchdbUtils.adapterFun(name, function (...args) {
      setup().then(function () {
  ...
  async function fetchJSON(url, options) {
    ...
    if (!result.ok) {
      ...
      const err = pouchdbErrors.generateErrorFromResponse(result.data);
      throw err;
    }
  ...
  api.bulkGet = pouchdbUtils.adapterFun('bulkGet', function (opts, callback) {
    ...
    function doBulkGetShim() {
      ...
      for (let i = 0; i < numBatches; i++) {
        const subOpts = pouchdbUtils.pick(opts, ['revs', 'attachments', 'binary', 'latest']);
    ...
    const fetched = function (err, res) {
      ...
      if (res && res.results) {
        ...
        res.results = res.results.filter(function (c) {
          leftToFetch--;
          const ret = pouchdbUtils.filterChange(opts)(c);
          if (ret) {
            if (opts.include_docs && opts.attachments && opts.binary) {

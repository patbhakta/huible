...
class ChatNotifier extends StateNotifier<ChatState> {
  ...
  // Stash for the consent retry — set when a 409 is received.
  ConsentRequiredError? _pendingConsentError;
  String? _pendingLastUserMessage;
  ...
  Future<void> _streamReply({
    ...
    try {
      ...
      // Stash the error and the last user message for the retry.
      _pendingConsentError = e;
      _pendingLastUserMessage = text;
      state = state.copyWith(
        isSending: false,
        error: e,
  ...
  void clearError() {
    ...
    _pendingConsentError = null;
    _pendingLastUserMessage = null;
  }

  /// Called after the user acknowledges the consent card.
  ///
  /// (a) POSTs the real consent via the data source using the stashed
  ///     [ConsentRequiredError.conversationId] / cardVersion / acknowledgeUrl.
  /// (b) On success, ADOPTS the server-issued conversationId as [_conversationId].
  /// (c) Re-sends the last user message (existing retry logic).
  /// On failed ack, surfaces a [GenericChatError]; does NOT retry.
  Future<void> acknowledgeConsent() async {
    final consentError = _pendingConsentError;
    final lastMessage = _pendingLastUserMessage;

    // Resolve the conversation_id to adopt (from the 409 body).
    final serverConversationId = consentError?.conversationId;
    final cardVersion = consentError?.cardVersion ?? 3;
    final ackUrl = consentError?.acknowledgeUrl;

    // Clear stash immediately so we don't double-fire.
    _pendingConsentError = null;
    _pendingLastUserMessage = null;
    state = state.copyWith(clearError: true, clearConsent: true);
    ...
    bool ackOk = false;
    try {
      ackOk = await _dataSource.acknowledgeConsent(
        conversationId: serverConversationId ?? _conversationId ?? '',
        cardVersion: cardVersion,
        acknowledgeUrl: ackUrl,
      );
    } catch (e) {
      // Ack failed — surface as generic error, do NOT retry.
      final msg = e is ChatError && e is GenericChatError
          ? e.message
          : (e is AuthRequiredError
              ? e.message
              : (e is ForbiddenError ? e.message : e.toString()));
      state = state.copyWith(error: GenericChatError(msg));
      return;
    }

    if (!ackOk) {
      state = state.copyWith(
        error: const GenericChatError('Consent acknowledgment was not accepted by the server.'),
      );
      return;
    }

    // (b) Adopt the server-issued conversationId.
    if (serverConversationId != null) {
      _conversationId = serverConversationId;
    }

    // (c) Re-send last user message.
